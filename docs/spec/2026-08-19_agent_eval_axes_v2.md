...
6つのスコアを1つにする**合成式・欠測時の重み再正規化・Tier は §12**（Q-9 確定）。重み表だけでは総合点は出せない。
...
## 12. 総合点の合成と欠測時の再正規化（Q-9 確定・2026-08-20）

§2.1 に重み表はあったが、6つのスコアを1つにする式と、軸が欠測したときの扱いが無かった。S12 の唯一のブロッカー。以下で確定する。

### 12.0 3つの原則

**P-A 欠測は0点にせず、最良値でも補完しない。** v1 §13 は前者だけを禁じていた。後者を禁じないと「測れない環境ほど高く出る」形が残る。これは本製品が最初に否定したもの（設定を凝ると点が上がる装置）と同型になる。現行仕様にこの形が2箇所ある（§12.8）。

**P-B「事象が無い（0/0）」と「測れない（小分母）」を分ける。** 両方を欠測に畳むと、書き直しが1件も無い＝最良の環境が総合点から消える。

**P-C 比較は同じ軸集合の上でしか成立しない。** 4軸で出た総合点と6軸で出た総合点の差は、環境の変化ではなく軸集合の変化を測っている。

### 12.1 合成式

```
SCORED  = { a ∈ 6軸 : availability(a) == "available" }   # 率が存在する軸
MISSING = 6軸 \ SCORED

if |MISSING| >= 3   → composite = null（suppressedReason = "too-many-missing-axes"）

W                  = Σ_{a∈SCORED} w_a               # 公称重みの和
composite          = Σ_{a∈SCORED} w_a · score_a / W # 0-100
effectiveWeight(a) = 100 · w_a / W                  # 提出・表示とも必須。合計 100
```

再正規化は「残った軸の重みを、和が100になるまで比例で引き上げる」だけ。加点も減点もしない。

🚨 **`belowMinDenominator` は合成の可否に使わない**（Leon #18 の分離に合わせた）。初稿は `availability == "available" ∧ belowMinDenominator == false` を条件にしていたが、これは実装が意図的に分けた2つの問いを混ぜ直すものだった。**「率が存在するか」と「窓間の差を改善と呼べるか」は別の問い**で、後者を律するのがクラスタ最小値。8,502件のツール呼び出しがある環境を「測るものが無い」と報告したのが、混ぜた場合に起きることだった。

`belowMinDenominator == true` の軸は**合成には入るが、CI を持たない**。その影響は §12.6（Δを改善と呼べる条件）で受ける。合成から外すのではなく、**言えることの範囲を狭める**方向で効かせる。

### 12.2 欠測の上限を2軸にする（v1 の3軸から変更）

v1 は8軸で「欠測3軸超は出さない」＝ 3/8 = 37.5% まで許容していた。6軸で同じ比率を保つと 2.25 軸 → **2軸**。境界の実測:

| 欠測 | 残る公称重みの最小 | 1軸が総合点に占める最大シェア |
|---|---|---|
| 1軸 | 81.25 | 22.73% |
| 2軸 | 63.75 | 28.85% |
| 3軸 | 46.25 | **39.47%** |

3軸欠測を許すと、総合点の約4割が1軸で決まる窓が出る（軸2・3・4が欠けたとき）。軸1・2・3が欠けた環境では残重み 46.25 で、半分以下の重みから100点満点を名乗ることになる。2軸までなら最悪でも 63.75 が残り、単軸シェアは3割を超えない。

### 12.3 総合点を出さない条件（既存の門番を1箇所に集めたもの）

| # | 条件 | 出典 | `suppressedReason` |
|---|---|---|---|
| 1 | `linesParseFailed / linesRead > 0.05` | §3.0 | `parse-failure-rate` |
| 2 | `rootsWalked` に `subagents/**` が無い | §3.0 | `subagents-not-walked` |
| 3 | `activeDays < 5` | §3.0 | `too-few-active-days` |
| 4 | `|MISSING| >= 3` | 本節 | `too-many-missing-axes` |

**#1〜#3 の3語は実装が既に持っている `GateReason` の実文字列**（`src/score/gate.ts`）。初稿では `parse_rate` / `roots_incomplete` / `active_days` という別名を新設していたが、それは AB-88 で指摘されたのと同じ形（同じものを2つの文書が別の名前で呼び、受け取り側が黙って比較に失敗する）なので実装側に合わせた。**新設したのは `too-many-missing-axes` の1語だけ。**

現状 `GateReason` はワイヤに出ていない（`gate()` の `totalAllowed` だけが使われ、理由は捨てられている）。`suppressedReason` はこの3語をワイヤに載せる場所でもある。「総合点が無い」ことは今も送れるが、「なぜ無いか」は送れていない。

v1 §4 の「整合性スコア 60未満」は**実装が無い**（整合性スコアそのものが未実装）。実装された時点で `integrity-below-threshold` を union に足す。無い門番を仕様の表に書いて実装済みのように見せない。

`composite.score` が null のときは `suppressedReason` を必ず埋める（閉じた union）。**null と 0 を型で区別する。**

🚨 **Leon 環境は現在6軸すべてが not_applicable（最大クラスタ11 < 最小分母20）なので #5 で null が正解であり、これが第1弾の正常系である。** 総合点が出ないことをテストの失敗として扱わない。

### 12.4 総合点が出ないときに何を出すか

軸別スコアの内訳、各軸の availability と欠測理由、実名の指摘リスト、Δ（§12.6 の条件を満たす軸のみ）。v1 §17 のとおり第1弾は総合点を主表示にしない。総合点は内訳の後に置く。

### 12.4b 総合点は「どちらに傾いているか」を必ず一緒に出す

各軸は落ちた項のぶんだけ真値からずれる（§12.5 の `omittedTerms.leans`）。**加重平均はそのずれも平均する**ので、総合点は素の値に見えて系統的に高め（または低め）になりうる。

SCORED の軸に `omittedTerms` が1つでもあるなら、総合点の隣に集約した傾きを必ず出す。

```
composite.leans = "high"      # 実効重みで見て、high に傾く項の重みが優勢
                = "low"
                = "mixed"     # 両方向が混在し、どちらとも言えない
                = null        # SCORED の全軸に omittedTerms が無い
```

例。第1弾の軸2は正の分子項を3つ落としていて score が高めに出る一方、winsorisation を落としているぶんは低めに出る（#17）。この2つが同居する軸を含む総合点は `mixed` であって、`high` でも `low` でもない。**どちらとも言えないことを言えないと書く**のが正しい状態で、精度が上がったふりをしない。

### 12.5 軸の中で項が落ちたとき

軸内の再正規化は式の形で扱いが変わる。一律にすると符号がおかしくなる。

**(a) 凸結合の軸（軸2・3・4・6）** — 係数の和が1。項を落としたら、残りの係数を和が1になるまで比例で引き上げる。スコアの意味は保たれる。

```
軸3: 0.60·V_eff + 0.30·S + 0.10·(1 − 未解決率)
     S が最小分母割れ → 0.857·V_eff + 0.143·(1 − 未解決率)
```

（上は再正規化の**算術の例示**。第1弾で実際に落ちうる項は §3.4 が明示している**軸4の (c) 人手上書き率だけ**で、`droppedTerms` の union もそれ1語で始める。落ちうる項を増やすには仕様の追記と union の拡張が要る＝黙って項が消えない。）

**(b) 減点式の軸（軸1・軸5）** — 項を落とすとスコアは必ず上がる。係数を残りに配分すると別の項が実態以上に効く。**どちらもしない。項が落ちた減点式の軸は `available` にせず not_applicable にして総合点から外す。**

軸1の I は減点予算 4+30+8+2 = 44 点のうち **30点＝68.2%** を占める。落として出す数字は、同じ名前の別の量である。

落ちた項は `axes.<name>.omittedTerms` で提出する。これが無いと、2環境の同名スコアが別の式から出ていても受け取り側で気づけない。**自由文字列にはしない**（ペイロードの葉から自由 `string` を型で排除した方針に従う）。

名前は実装の `OMITTED_TERMS`（`src/score/wastedMotion.ts`）に合わせた。ただし**原因が2種類ある**ので、ワイヤでは分けて持つ:

| `cause` | 意味 | 例 |
|---|---|---|
| `not-implemented` | 仕様にはあるが実装が無い。**バージョンごとに固定** | 軸2の `unused-success` / `verification-exclusion` / `user-script-decay` / `winsorisation` |
| `below-minimum` | 実装はあるが、この窓のデータが最小分母を満たさない。**窓ごとに変わる** | 軸4の `axis4-manual-overwrite` |

🚨 **各項には `leans: "high" | "low"` を必ず添える**。#17 が「落とした項がどちらに傾くか」を言語で書いたのと同じことを、機械可読な形で載せる。軸2は正の分子項を3つ落としているので **score は高めに出る**、winsorisation を落としているぶんは低めに出る、という情報が無いと、受け取り側は「57.1」を素の値として読む。**「未実装」だけでは、読み手が真値のどちら側にいるか分からない**（#17 の原文どおり）。

### 12.6 前窓Δ・対照基準との比較（P-C の実装）

Δを出してよいのは、**両方の窓が同一の軸集合で計算されている**ときだけ。集合が違うときは共通部分で両方を計算し直し、`basis: "intersection"` と使った軸を明示する。共通部分が **4軸未満ならΔを出さない**（総合点の可視条件と同じ 63.75 の下限を使う）。

有意判定は §6.3 に従う。CI は**セッション単位のリサンプルを1回引き、その同じリサンプルで6軸を再計算して**加重平均の分布から取る。軸ごとに別々のリサンプルを引くと軸間の相関が消え、幅が過小になる。

🚨 **SCORED の軸に1つでも `confidenceInterval: null`（＝`belowMinDenominator: true`）があれば、composite の CI も null**。総合点そのものは出るが、**Δを「改善」「悪化」と呼ぶことはできない**（§6.3 の第1条件が評価できないため、必ず「変化なし（誤差不明）」と表示する）。これが §12.1 で合成から外さない代わりに置いた制約で、クラスタ最小値が本来 governs していた問いはここで効く。

MDE_floor は **1.0pt**。0-100 に変換した後は全軸が同一スケールに載るため、§6.3 の率系 floor をそのまま使う（軸5の「前窓比10%」は 0-100 化する前の個数系指標に対する規定であり、合成後には適用しない）。

Δには必ず寄与分解を併示する。「総合 +4.7pt のうち、軸2 が +3.1pt、軸4 が +1.8pt、軸1 が −0.2pt」。総合点だけが動いて理由が言えない表示にしない。

対照基準（素の環境）との比較も同じ規則。素の環境は軸5・軸6が欠測になりやすいので、**対照との差は必ず共通軸で取る**。

### 12.7 2次元表示（帰結 × 設計）

v1 §13 は〈帰結＝軸1-4〉×〈設計＝軸5-8〉だった。軸7・軸8を落としたので設計側は2軸しか残らない。

- **帰結スコア** = 軸1・2・3・4 の加重平均（公称 71.25 を100に再正規化）。4軸中 **3軸以上**が SCORED のときのみ表示
- **設計スコア** = 軸5・軸6 の加重平均（公称 28.75）。**2軸とも SCORED のときのみ表示**。片方が欠測なら、残り1軸を「設計スコア」と名乗らせない（§12.5 と同じ理由。名前が同じで中身が別の量になる）
- どちらかが表示できない窓では2次元散布そのものを出さず、軸別内訳に落とす

### 12.8 現行仕様の2箇所を訂正する（P-A 違反）

**(1) 軸1の I（§3.1 / v1 §7）** — 「分母が10未満なら I は欠測」しか書いておらず、0/0 と小分母が同じ扱いになっている。

- `Σ(depth−1) == 0` → **I = 0**。再書き込みが1件も無いのだから誘発再編集も存在しない。減点なしが正しい
- `1 ≤ Σ(depth−1) < 10` → I 欠測 → §12.5(b) により軸1を not_applicable

**(2) 軸5の U（§3.5 / v1 §9）** — 「資産数3未満で U は欠測（台形スコアのみ）」は、乗数 `(0.5 + 0.5·U)` を 1.0 に置くこと、すなわち**最良値での補完**になっている。同じ「1本も発火していない」状態で:
...
資産を2本に減らすだけで軸5のスコアが **2.0倍**になる。訂正:

- 資産数 0 → 乗数を掛けない（持っていないものは使えない。台形スコアのみ）
- 資産数 1〜2 → **U を実際に計算する**（最小分母3を撤廃）。U は率でなく平均なので分母2でも定義でき、幅の広さは CI 側で表現する

この2件は §12.1 の前段にあるので、直さないと総合点の値そのものが変わる。
...
軸1・軸3の絶対閾値（§3.1 / §3.3）と同じ刻みを使う。1製品に2つの5段階を持ち込まない。`composite.score` が null の窓では Tier も null。母集団への言及は段階1では一切しない（v1 §14）。

### 12.10 提出ペイロードへの追加

`composite` ブロックと `axes.<name>.droppedTerms` を新設する。検算に必要な材料（`axesUsed` / 実効重み / 除外理由）を全部載せ、受け取り側が再計算して照合できる形にする。**総合点は送信側の計算を信じる対象ではなく、受信側が再計算して一致を確かめる対象。** 詳細と新規バリデーション（V-17〜V-21 / W-8）は提出ペイロード仕様 §4.4 と §6。
